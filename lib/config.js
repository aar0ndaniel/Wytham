...
const DEFAULT_SUPPORT_EMAIL = 'aaronakuteye@gmail.com';
const DEFAULT_SUPPORT_BCC_EMAIL = 'hbessel.art@knust.edu.gh';
...
function createConfig(env = process.env) {
  ...
  const supportBccEmail = DEFAULT_SUPPORT_BCC_EMAIL;
  const adminUsername = trim(env.ADMIN_USERNAME) || 'admin';
  const adminPassword = trim(env.ADMIN_PASSWORD) || 'change-this-password';
  ...
  return {
    ...
    adminUsername,
    adminPassword,
    adminUsers: adminUsers(env.ADMIN_USERS, adminUsername, adminPassword),
    ...
    supportEmail,
    supportBccEmail,
    supportUpdateEmail: supportEmail,
    liteShareUrl:
...
function adminUsers(value, fallbackUsername, fallbackPassword) {
  const fallback = [{ username: fallbackUsername, password: fallbackPassword }];
