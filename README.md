...
SMTP remains as a fallback for environments that allow outbound SMTP ports. The sender fills `signup-beta-email-template.html`; production email embeds the dark Metis logo as an inline CID image so email clients do not depend on loading an external logo URL.
