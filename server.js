...
app.post('/api/signup', async (req, res) => {
  ...
  res.json({
    ...
    message: emailResult.status === 'sent'
      ? `Thank you, ${firstName(signup.name)}. Please check your email for your metis beta access link.`
      : `Thank you, ${firstName(signup.name)}. We saved your request and will send your metis beta email shortly.`,
  });
});
...
function startServers() {
  ...
  adminApp.listen(config.adminPort, config.adminHost, () => {
    console.log(`metis admin dashboard listening on http://${config.adminHost}:${config.adminPort}/admin`);
  });
}
  ...
  <div class="shell">
    <div class="card">
      ...
      <div class="meta">
        ...
        <strong>Support:</strong> <a href="mailto:${escapeHtml(currentConfig.supportEmail || currentConfig.smtpFromEmail || '')}?subject=metis%20support">${escapeHtml(currentConfig.supportEmail || currentConfig.smtpFromEmail || 'Not configured')}</a>
      </div>
    </div>
  </div>
</body>
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <meta name="description" content="metis Beta Operations Admin Dashboard" />
        ...
        <div class="hero-brand">
          <div class="brand-wrap">
            ...
            <div class="brand-stack">
              <div class="hero-eyebrow">metis control room</div>
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>metis Admin &mdash; Sign in</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  ...
  <div class="login-wrap">
    <div class="login-mark">
      <div class="login-badge">
        <img src="/admin/logo" alt="metis icon" />
      </div>
      <div class="login-kicker">metis admin</div>
    </div>
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>metis Admin &mdash; Email Preview</title>
  <style>
...
<body>
  <div class="shell">
    <div class="toolbar">
      ...
      <div class="preview-title">${adminIcon('preview')}<span>metis email preview</span></div>
    </div>
...
function setAdminSession(res, username) {
  res.setHeader('Set-Cookie', serializeCookie('metis_admin', createAdminSession(username), {
    httpOnly: true,
    sameSite: 'Strict',
    path: '/admin',
    maxAge: 8 * 60 * 60,
...
function createApp(options = {}) {
  ...
  hostedApp.post('/api/signup', async (req, res, next) => {
    try {
      ...
      return res.json({
        success: true,
        message: `Thank you, ${firstName(savedSignup?.name || body.name)}. We saved your request and will send your metis beta email shortly.`,
      });
    } catch (error) {
      return next(error);
    }
