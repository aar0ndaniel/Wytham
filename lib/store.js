const SIGNUP_EMAIL_COLUMNS = 'token,email_status,email_error,email_sent_at,email_sent_by';
const LEGACY_SIGNUP_EMAIL_COLUMNS = 'token,email_status,email_error,email_sent_at';
const RECENT_SIGNUP_COLUMNS =
  'token,name,email,institution,country,role,edition,created_at,updated_at,email_status,email_sent_by,beta_visits,last_beta_visit_at';
const LEGACY_RECENT_SIGNUP_COLUMNS =
...
const EXPORT_SIGNUP_COLUMNS = 'name,email,institution,country,role,edition,created_at,email_status,email_sent_by,beta_visits';
const LEGACY_EXPORT_SIGNUP_COLUMNS = 'name,email,institution,country,role,edition,created_at,email_status,beta_visits';
...
function createStore(client, options = {}) {
  ...
  return {
    ...
    listRecentSignups(limit = 50) {
      return withMissingSentByFallback(
        client.from(tables.signups).select(RECENT_SIGNUP_COLUMNS).order('updated_at', { ascending: false }).limit(limit),
        () => client.from(tables.signups).select(LEGACY_RECENT_SIGNUP_COLUMNS).order('updated_at', { ascending: false }).limit(limit)
      );
    },

    listSignupsForEmail(limit = 5000) {
      return withMissingSentByFallback(
        client.from(tables.signups).select(EMAIL_SIGNUP_COLUMNS).order('updated_at', { ascending: false }).limit(limit),
        () => client.from(tables.signups).select(LEGACY_RECENT_SIGNUP_COLUMNS).order('updated_at', { ascending: false }).limit(limit)
      );
    },
    ...
    listSignupsForExport(limit = 1000) {
      return withMissingSentByFallback(
        client.from(tables.signups).select(EXPORT_SIGNUP_COLUMNS).order('created_at', { ascending: false }).limit(limit),
        () => client.from(tables.signups).select(LEGACY_EXPORT_SIGNUP_COLUMNS).order('created_at', { ascending: false }).limit(limit)
      );
    },
    ...
    markSignupEmailStatus(token, { error = '', sentAt = '', sentBy = '', status } = {}) {
      const statusPatch = {
        email_error: error,
        email_sent_at: normalizeTimestamp(sentAt),
        email_sent_by: String(sentBy || '').trim(),
        email_status: normalizeEmailStatus(status),
      };
      const legacyStatusPatch = { ...statusPatch };
      delete legacyStatusPatch.email_sent_by;
      return withMissingSentByFallback(
        client
          .from(tables.signups)
          .update(statusPatch)
        ...
        () => client
          .from(tables.signups)
          .update(legacyStatusPatch)
          .eq('token', token)
          .select(LEGACY_SIGNUP_EMAIL_COLUMNS)
          .single()
      );
    },

    markSignupEmailSender(token, sentBy = '') {
      return ignoreMissingSentBy(
        client
        .from(tables.signups)
        .update({ email_sent_by: String(sentBy || '').trim() })
        .eq('token', token)
        .select('token,email_sent_by')
        .single(),
        { token, email_sent_by: '' }
      );
    },
...
async function withMissingSentByFallback(resultLike, fallbackQuery) {
  const result = await resultLike;
  if (!missingSentByColumn(result?.error)) {
    return addMissingSentBy(result);
  }
  return addMissingSentBy(await fallbackQuery());
}

async function ignoreMissingSentBy(resultLike, fallbackData) {
  const result = await resultLike;
  if (missingSentByColumn(result?.error)) {
...
function missingSentByColumn(error) {
  if (!error) return false;
  const text = String(error.message || error.hint || error.details || '');
  if (!/email_sent_by/i.test(text)) return false;
  return error.code === '42703' || error.code === 'PGRST204' || error.code === 'PGRST205';
}

function addMissingSentBy(result) {
  if (Array.isArray(result?.data)) {
    return {
      ...result,
      data: result.data.map((row) => (
        Object.prototype.hasOwnProperty.call(row, 'email_sent_by') ? row : { ...row, email_sent_by: '' }
      )),
    };
  }
  if (result?.data && !Object.prototype.hasOwnProperty.call(result.data, 'email_sent_by')) {
    return { ...result, data: { ...result.data, email_sent_by: '' } };
  }
  return result;
}
